...
# @permission_required('network.can_access_alumni_content', raise_exception=True)
def index(request):
...
def register(request):
    if request.method == "POST":
        ...
        try:
            ...
            # Check user group and assign to corresponding group
            user_group = request.POST.get("user_group")
            if user_group not in ["Alumni", "Eleve"]:
                return render(request, "network/register.html", {
...
def create_post(request):
    if request.method == 'POST':
        # Vérifiez si l'utilisateur appartient au groupe "Eleve"
        if request.user.groups.filter(name='Eleve').exists():
            # Redirigez l'utilisateur ou affichez un message d'erreur
            messages.error(request, 'Les élèves ne sont pas autorisés à créer des messages.')
            return redirect('index')
        
        # Si l'utilisateur n'est pas du groupe "Eleve", continuez avec la création du post
...
def upload_cv(request):
    ...
    if request.method == 'POST':
        ...
        nouveau_cv = CV(fichier=cv_file)
        nouveau_cv.save()
        cv = nouveau_cv.fichier.url  # Récupérer l'URL du fichier CV
    return render(request, 'network/cv.html', {'cv': cv})



def events_page(request):
    # Logique pour récupérer les événements à afficher sur la page
    events = [...]  # Remplacer [...] par la logique pour récupérer les événements
    return render(request, 'network/events.html', {'events': events})
