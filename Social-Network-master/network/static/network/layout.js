...
function confirm_delete(id) {
  ...
  document.querySelector("body").style.overflow = "hidden";
  small_popup
    .querySelector("#delete_post_btn")
    .setAttribute("onclick", `delete_post(${id})`);
}
...
function edit_post(element) {
  ...
  promise.then(() => {
    ...
    popup
      ...
      .setAttribute(
        "onsubmit",
        `return edit_post_submit(${post.dataset.post_id})`
      );
    popup.querySelector(".submit-btn").disabled = false;
  });
}
...
function login_popup(action) {
  ...
  if (action === "like") {
    ...
    document.querySelector(".main_text-div").querySelector("h2").innerText =
      "Save a post to reference later";
    ...
    document.querySelector(".main_text-div").querySelector("h2").innerText =
      "Follow people that inspires you";
  }
}
...
function previewFile() {
  ...
  if (file) {
    ...
    //});
    document
      .querySelector(".form-action-btns")
...
function follow_user(element, username, origin) {
  ...
  fetch("/" + username + "/follow", {
    ...
    if (document.querySelector(".body").dataset.page === "profile") {
      if (
        document.querySelector(".profile-view").dataset.user ===
        document.querySelector("#user_is_authenticated").dataset.username
      ) {
...
function display_comment(comment, container, new_comment = false) {
  let writer = document.querySelector("#user_is_authenticated").dataset
    .username;
